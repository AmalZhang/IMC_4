...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        late_phase = state.timestamp >= 85000
        ...
        if self.HYDROGEL in state.order_depths:
            fair = memory.get(f"ema_{self.HYDROGEL}", 10000.0)
            result[self.HYDROGEL] = self.trade_delta_one(
                self.HYDROGEL,
                state.order_depths[self.HYDROGEL],
                state.position.get(self.HYDROGEL, 0),
                fair,
                take_edge=10.0 if not late_phase else 8.0,
                make_edge=9.0 if not late_phase else 6.0,
                skew=0.14 if not late_phase else 0.32,
                max_make_qty=14 if not late_phase else 10,
                max_take_qty=25 if not late_phase else 18,
                quote_abs_limit=100 if not late_phase else 70,
            )

        spot_mid = None
        if self.VELVETFRUIT in state.order_depths:
            spot_mid = self.mid_price(state.order_depths[self.VELVETFRUIT])
            # Keep underlying focused on option hedge; avoid running an independent
            # market-making strategy that can conflict with hedge inventory.
            result[self.VELVETFRUIT] = []
    ...
    def trade_delta_one(
        ...
        max_make_qty: int = 20,
        max_take_qty: int = 40,
        quote_abs_limit: Optional[int] = None,
    ) -> List[Order]:
        orders: List[Order] = []
        limit = self.LIMITS[product]
        pos = position
        quote_limit = limit if quote_abs_limit is None else min(limit, quote_abs_limit)
        skewed_fair = fair - skew * pos

        for price, volume in sorted(depth.sell_orders.items()):
            if price > skewed_fair - take_edge or pos >= limit:
                break
            qty = min(-volume, limit - pos, max_take_qty)
            if qty > 0:
        ...
        for price, volume in sorted(depth.buy_orders.items(), reverse=True):
            if price < skewed_fair + take_edge or pos <= -limit:
                break
            qty = min(volume, pos + limit, max_take_qty)
            if qty > 0:
                orders.append(Order(product, price, -qty))
                pos -= qty
        ...
        if bid_price < ask_price:
            buy_qty = min(max_make_qty, max(0, quote_limit - pos))
            sell_qty = min(max_make_qty, max(0, pos + quote_limit))
            if buy_qty > 0:
    ...
    def trade_option(
        ...
        risk_limit = self.OPTION_MAX_ABS_POS[product]
        skewed_fair = fair - 0.025 * pos

        for price, volume in sorted(depth.sell_orders.items()):
            if price > skewed_fair - take_edge or pos >= risk_limit:
                break
            qty = min(-volume, risk_limit - pos, 22)
            if qty > 0:
                orders.append(Order(product, price, qty))
                pos += qty
        ...
        for price, volume in sorted(depth.buy_orders.items(), reverse=True):
            if price < skewed_fair + take_edge or pos <= -risk_limit:
                break
            qty = min(volume, pos + risk_limit, 22)
            if qty > 0:
                orders.append(Order(product, price, -qty))
                pos -= qty
                delta_change -= qty * delta

        # Avoid providing too much optionality in wings and around ATM.
        ...
        ask_price = max(best_ask - 1, math.ceil(skewed_fair + make_edge))
        if bid_price < ask_price:
            buy_qty = min(16, max(0, risk_limit - pos))
            sell_qty = min(16, max(0, pos + risk_limit))
            if buy_qty > 0:
                orders.append(Order(product, bid_price, buy_qty))
            if sell_qty > 0:
                orders.append(Order(product, ask_price, -sell_qty))
